...
reference_data = prepare_data(pd.read_parquet('data/reference.parquet'))
with open('models/lin_reg.bin', 'rb') as f_in:
	model = joblib.load(f_in)

raw_data = prepare_data(pd.read_parquet('data/green_tripdata_2024-03.parquet'))
...
def calculate_metrics_postgresql(curr, i):
	...
	majority_std_error = result['metrics'][4]['result']['current']['underperformance']['majority']['std_error']

	curr.execute(
		"insert into dummy_metrics(timestamp, prediction_drift, num_drifted_columns, share_missing_values, fare_amount_quantile, majority_std_error) values (%s, %s, %s, %s, %s, %s)",
		(begin + datetime.timedelta(i), prediction_drift, num_drifted_columns, share_missing_values, fare_amount_quantile, majority_std_error)
	)
